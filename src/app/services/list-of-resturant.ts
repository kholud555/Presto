...
import {
  RestaurantItem,
  ResturantInterface,
} from '../models/ResturantInterface/resturant-interface';
...
export class ListOfResturant {
  ...
  getItemsByRestaurantName(
    name: string
  ): Observable<{ $values: RestaurantItem[] }> {
    return this.http.get<{ $values: RestaurantItem[] }>(
      `${this.itemsApiUrl}?restaurantName=${name}`
    );
  }
